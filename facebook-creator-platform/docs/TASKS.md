...
- [ ] **T4.1 Search service** (~4h) — scaffold `services/search/` (full NestJS app, no Postgres schema — Algolia is the system of record, ADR-004); four idempotent consumers:
  ...
  No HTTP call back to `apps/api` — event payloads carry all indexable data (ADR-051/CQRS). HTTP `GET /workspaces/:id/search?q=` passthrough in `apps/api` calls `services/search` which proxies to Algolia. DoD: all four events trigger correct Algolia op; search returns hits; duplicate event is deduped by `eventId` (Redis NX); tests for all four consumers.
- [ ] **T4.2 Notification service** (~5h) — scaffold `services/notification/` (full NestJS app, `notification` schema); two internal tables: `notifications` + `notification_recipients` (from DDL) + `workspace_members_projection` (CQRS projection — ADR-052); entities do NOT extend `BaseEntity` (DDL has no `updatedAt`/`deletedAt` on these tables).
  ...
  **HTTP endpoints** (called by `apps/api`): `GET /workspaces/:id/notifications` (recipient's unread + recent); `PATCH /notifications/:id/read`.
  DoD: projection stays consistent across all membership events; all notification events produce rows + recipients; Slack alert fires on key events; read endpoint returns correct rows; `PATCH` marks read (one-way); BR-F08 revert attempt → err; tests for projection consumers + notification consumers + HTTP endpoints.
- [ ] **T4.3 Email service** (~6h) — scaffold `services/email/` (full NestJS app, `email` schema). `EmailDeliveryLog` entity does NOT extend `BaseEntity` (DDL has `sent_at`/`created_at`, no `updatedAt`/`deletedAt`).
  ...
  **Consumers** — mapped to DDL `email_type CHECK ('invitation','publish_success','publish_failed','token_expiring','payment_failed')`:
  ...
  - `posts.published` → `email_type='publish_success'`; recipient from `GET /internal/users/:id/email` using `createdByUserId` (ADR-050); `related_entity_type='post', related_entity_id=postId`
  - `posts.failed` → `email_type='publish_failed'`; recipient from `GET /internal/users/:id/email` using `createdByUserId`; `related_entity_type='post', related_entity_id=postId`
  - `billing.payment_failed` → `email_type='payment_failed'`; recipient from `GET /internal/workspaces/:id/owner-email` (ADR-050); `related_entity_type='workspace', related_entity_id=workspaceId`
  - `facebook.token_expiring` → `email_type='token_expiring'`; recipient from `GET /internal/workspaces/:id/owner-email`; `related_entity_type='facebook_account', related_entity_id=facebookAccountId`
...
- [ ] **T5.2 Schema audit** (~2h) — no DB id `DEFAULT` on service-owned tables; all `BaseEntity` subclasses have the `createdAt`/`updatedAt`/`deletedAt` triplet; entities intentionally excluding `deletedAt` (WorkspaceMember, Invitation, Plan, Subscription, BillingEvent, service tables matching DDL) are documented exceptions; run `docs/reference/verify-seed.sql` after commenting out rows for `core.audit_logs` (ADR-047) and `messaging.*` (see T5.8). DoD: all active checks pass; no undocumented exceptions.
- [ ] **T5.8 `messaging` schema tables** (~2h) — create `messaging.event_message_logs` + `messaging.dead_letter_messages` in a new migration (per `fcp-ddl.sql`); wire publisher to write a `pending` row before sending and update it to `processed` after ack (or `dlq` on permanent nack); update `verify-seed.sql` row-count section to include these tables. DoD: migration runs cleanly; DLQ writes a `dead_letter_messages` row; verify-seed.sql rows reference `messaging.*` without error.
