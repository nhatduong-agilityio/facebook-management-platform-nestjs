...
## ADR-102 — M-11 Partial indexes for quota count and publish cron (2026-07-17)

Two partial indexes added in `Migration20260717000001_PerfIndexesV2`.

**`idx_posts_workspace_active` — `(workspace_id) WHERE deleted_at IS NULL`**
`MikroOrmPostRepository.countByWorkspace` issues `COUNT(*) WHERE workspace_id = ? AND deleted_at IS NULL`
on every `POST /posts` request (plan-quota check, CODING-STANDARDS §5 R8). Without a partial index
Postgres must scan all workspace rows and filter soft-deleted ones in memory after the index scan.
The partial predicate confines the index to active rows only, keeping it small as the post table grows.

**`idx_posts_scheduled_due` — `(status, scheduled_at) WHERE status = 'scheduled' AND deleted_at IS NULL`**
`PublishJob` queries `WHERE status = 'scheduled' AND scheduled_at <= now() AND deleted_at IS NULL`
every minute. The composite `(status, scheduled_at)` enables an index range scan on `scheduled_at`
after the equality filter on `status`. The partial predicate eliminates all non-scheduled and
soft-deleted rows from the index, so the index stays small regardless of historical post volume.

**Why separate from `Migration20260715000001_PerfIndexes`:**
M-11 was identified after T5.6 (which produced the T5.6 migration). Adding new indexes to a
previously applied migration would require re-running the down/up cycle in existing environments.
A new migration is the correct approach.

**EXPLAIN ANALYZE confirmation:** must be run against a live Postgres 16 instance after
`migration:up`. Expected: both queries show `Index Scan using idx_posts_workspace_active / idx_posts_scheduled_due` in the plan.
...
| 2026-07-14 | **TR.9 complete.** `services/notification` hybrid app; 11 `@EventPattern` consumers; `NotificationMessagingModule` removed. Projection consumers (`MemberJoined`, `MemberRemoved`, `MemberRoleChanged`) gained `MikroORM` constructor param + `RequestContext.create` because `MikroOrmNotificationRepository` injects `EntityManager` directly and `allowGlobalContext: false` throws without a request context in microservice handlers. `MemberInvited` is dedup-only (no ORM path). 2 new specs created for `billing-subscription-cancelled` and `billing-subscription-past-due`. |
...
| 2026-07-17 | **ADR-107 `DevAuthModule` token compatibility with `ClerkAuthGuard` — verified, no fix needed (L-19).** Investigation: `DevAuthService.generateToken` calls `POST /v1/sessions/{id}/tokens` on the Clerk REST API, which returns a genuine Clerk-signed session JWT. `ClerkAuthGuard` verifies tokens via `verifyToken(token, { secretKey })` from `@clerk/backend` — a local crypto check (no Clerk API call) that validates the JWT signature against the same secret key used to sign Clerk session tokens. The tokens are cryptographically compatible; the guard accepts them in dev. No bypass mechanism is needed and **none has been added** (a bypass would be a production security risk). Two-step fallback: when the dev user has no active Clerk session, `DevAuthService` returns `{ loginUrl }` (a Clerk sign-in magic link); the developer visits it in a browser to create a session, then calls `POST /dev-auth/token` again to receive `{ accessToken }`. This is documented in `DevAuthService`'s JSDoc. `DevAuthModule` itself is already excluded from production builds via `NODE_ENV !== 'production'` guard in `AppModule`. |
| 2026-07-17 | **ADR-106 `PublishFallbackPollJob` threshold — 30 minutes (L-18).** `PUBLISH_TTL_MINUTES` defaults to 30. Rationale: Facebook's webhook retry cycle runs for ~25 minutes at exponentially increasing intervals. A 30-minute TTL means the fallback job only activates after Facebook's full retry window has likely closed, giving the `FacebookFeedConsumer` (webhook path) a generous head-start. This keeps the concurrent-transition race window negligible — the `em.find({ status: 'publishing' })` query is the primary guard: if the webhook already committed `published`, the post is absent from the result set. Any duplicate `PostPublishedEvent` that does escape concurrent timing is deduplicated by event consumers via Redis dedup keys (ADR-028/029). **TTL×3 (90 min default) failure threshold**: three full poll cycles without confirmation means the post is unlikely to recover; marking `failed` prevents permanent `publishing` limbo while tolerating transient Graph API errors. Override via `PUBLISH_TTL_MINUTES` env var if Facebook's retry behaviour changes or load-test data (T5.5) warrants adjustment. |
| 2026-07-17 | **ADR-105 Workspace soft-delete policy — Option B: deferred (L-17).** Option A (permanently out-of-scope) was rejected: workspace deletion is a standard SaaS user need (account closure, GDPR right-to-erasure requests). **Option B chosen: deferred.** Rationale: (1) the `posts` and `facebook_accounts` tables carry `ON DELETE RESTRICT` FKs on `workspace_id` — any workspace soft-delete must cascade via the domain layer, not the DB; (2) cancelling the Stripe subscription, soft-deleting child aggregates, and emitting `WorkspaceDeletedEvent` require careful ordering within a single `em.flush()` (§6); (3) a 90-day hard-purge retention job is needed for GDPR compliance — the pattern already exists (`PublishFallbackPollJob` / ADR-054 forked-EM pattern). The feature is architecturally well-understood and ready to implement when prioritised; it is not permanently blocked. Full scope is captured in placeholder task **W-1** in `docs/TASKS.md`. |
| 2026-07-17 | **ADR-104 API versioning strategy (L-16).** Current state: `api/v1` is encoded in `setGlobalPrefix('api/v1')` in `apps/api/src/main.ts`; NestJS `enableVersioning()` is not active. **Non-breaking changes** (additive fields, new optional query params, new endpoints): ship directly on `v1` — no version bump, no new prefix. **Breaking changes** (removed fields, changed semantics, incompatible request shapes): (1) enable `VersioningType.URI` in `main.ts` and change `setGlobalPrefix` to `'api'`; (2) decorate existing controllers with `@Version('1')` (or set `defaultVersion: '1'` via `app.enableVersioning`) so all current routes continue resolving as `api/v1/...`; (3) add `@Version('2')` only on the specific controller methods that introduce the breaking change — no full controller or module duplication; (4) `v1` variants of the changed routes must continue to work for **6 months** after the first `v2` route ships; (5) add a `Sunset: <RFC-1123-date>` response header to the `v1` path during the deprecation window so clients can plan migration. Rationale: path-prefix versioning (rather than header or query-param) keeps routes bookmarkable and Swagger-friendly. Method-level `@Version` avoids the maintenance burden of duplicating entire modules. |
| 2026-07-17 | **ADR-103 HTTP timeout config + fail-open quota strategy (M-14).** (1) `HTTP_CLIENT_TIMEOUT_MS` env var (default 5 000 ms) controls `AbortSignal.timeout` in both `FetchHttpClientAdapter` and `BillingHttpClientAdapter` — avoids thread-pool starvation under a stuck downstream service without hardcoding values in source. (2) Fail-open in `BillingQuotaAdapter`: when `billingClient.getPostLimit` throws `DownstreamServiceError`, return `FREE_PLAN_LIMIT = 10` and emit a `Logger.warn` — not silently, and not from the HTTP adapter. Rationale: fail-open is a quota *policy* decision; the HTTP adapter is a pure transport concern. Returning the free-plan limit during a billing outage is intentionally conservative — workspaces stay functional, and on-call engineers see the warning before an SLA breach. Unexpected (non-`DownstreamServiceError`) errors are re-thrown so they surface as 500s. |
