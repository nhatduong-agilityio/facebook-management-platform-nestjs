...
export class PostFailedEmailConsumer {
  ...
  async onPostFailed(
    ...
    if (!isNew) {
      this.logger.log({ eventId: data.eventId }, 'PostFailedEmailConsumer: duplicate, skipping');
      channel.ack(msg);
      return;
    }
    ...
    try {
      ...
      await RequestContext.create(this.orm.em, async () => {
        ...
        await this.emailProvider.send({
          to: recipientEmail,
          templateName: 'post-failed',
          data: { postId: data.postId, lastError: data.lastError ?? 'Unknown error' },
        });
        await this.emailLogRepo.updateSent(log.id, new Date());
      });

      this.logger.log({ eventId: data.eventId }, 'PostFailedEmailConsumer: sent');
      channel.ack(msg);
    } catch (err) {
